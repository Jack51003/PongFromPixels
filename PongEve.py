...
testCoefs = [
    np.array(
        [
            [0.38238344, 0.7515745, 0.29565119, 0.35490288, 0.97040034],
    ...
    np.array(
        [
            [0.25498077, 0.03853811, 0.76089995],
...
class Paddle:
    ...
    def __init__(self, y=300, yspeed=0, coefs=0, intercepts=0):
        self.y = y
        self.ylast = y - yspeed
        self.yspeed = yspeed
        self.alive = True
    ...
    def generateCoefs(self, layer_structure):
        ...
        for i in range(len(layer_structure) - 1):
            coefs.append(
                np.random.rand(layer_structure[i], layer_structure[i + 1]) * 2 - 1
            )
        return coefs
    ...
    def draw(self):
        if self.winner == False:
            # pygame.draw.rect(screen, BLACK, [22, self.y, 20, 100])
            pygame.draw.rect(screen, RED, [0, self.y, 20, 100])
        else:
            # pygame.draw.rect(screen, BLACK, [22, self.y, 20, 100-4])
            pygame.draw.rect(screen, BLUE, [0, self.y, 20, 100])
...
class Ball:
    ...
    def update(self, paddle):
        ...
        if (
            ...
            self.xspeed = self.xspeed * -1 + random.uniform(0, 1)
            self.yspeed = self.yspeed + (self.y - paddle.y - 50) / 10
...
def calculateOutput(input, layer_structure, coefs, intercepts, g="identity"):
    ...
    for k in range(len(reduced_layer_structure)):
        ...
        result = np.matmul(np.transpose(coefs[k]), previousLayer) + np.transpose(
            np.array([intercepts[k]])
        )
...
def displayNetwork(layer_sctructure, coefs=testCoefs, command=0):
    ...
    for i in range(layer_structure[-1]):
        ...
        screen.blit(
            text, [width + 50, (i + 1) * int(height / (layer_structure[-1] + 2))]
        )
    ...
    for i in range(len(circle_positions) - 1):
        for j, circle_pos in enumerate(circle_positions[i]):
            for k, circle_pos2 in enumerate(circle_positions[i + 1]):
                ...
                if thickness > 0:
                    pygame.draw.lines(
                        screen, BLUE, False, [circle_pos, circle_pos2], thickness
                    )
                else:
                    pygame.draw.lines(
                        screen, RED, False, [circle_pos, circle_pos2], -thickness
                    )
...
while not done:
    ...
    for i, paddle in enumerate(paddles):
        ...
        input = np.array(
            [[paddle.y, balls[i].x, balls[i].y, balls[i].yspeed, balls[i].xspeed]]
        )
        paddle.command = calculateOutput(
            input, layer_structure, paddle.coefs, paddle.intercepts
        )
    ...
    if still_alive == 0:
        ...
        for i in range(COUNT - 1):
            paddles.append(
                Paddle(
                    coefs=mutateCoefs(winner.coefs),
                    intercepts=mutateIntercepts(winner.intercepts),
                )
            )
            balls.append(Ball())
